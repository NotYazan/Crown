...
client.on(Events.MessageCreate, async message => {
    ...
    if(message.channel.id !== '1061988460233236480') return;
    if (command === 'link') {
        ...
        await client.fetchInvite(link).then(async (data) => {
            ...
            if (!anti) {
                await client.fetchInvite(link).then(async (serverlink) => {
                    ...
                    let time = new Date();
                    let avatar = await client.channels.fetch('1061237610149842964');//شات الفتارات
                    let server = await client.channels.fetch('1061237610149842964');//شات السيرفر
                    let logs = await client.channels.fetch('1061994466422046740');
